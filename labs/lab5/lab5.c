...
char **plan_route(Graph *gr, char *start, char *dest){
    ...
    while (strcmp(start,dest_node->station)!=0){
        my_routine[i] = copy_node(dest_node->station);
        dest_node = dest_node->prev;
        i++;
        my_routine = realloc(my_routine,sizeof(char*)* (i+1));
    }
    my_routine[i] = copy_node(start_node->station);
    return my_routine;
}
...
void update(Graph *gr, char *start, char *dest, int weight){
    ...
    if (weight == 0){
        ...
        if (find_dest == NULL){return;}
        if (find_start->edges == NULL){return;}

        // go over the edges
        Enode* current;
        current = find_start->edges;
        Enode* previous;
        previous = NULL;

        while(current != NULL && strcmp(current->vertex, dest)!= 0){
            previous = current;
...
void disrupt(Graph *gr, char *station){
    Vnode* free_the_station;
    free_the_station = find_node(gr,station);
